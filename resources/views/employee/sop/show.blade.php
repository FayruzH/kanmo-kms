...
<div class="container-fluid px-0">
    <a href="{{ route('employee.dashboard') }}" class="text-decoration-none text-secondary small mb-3 d-inline-block">
    ...
    <div class="kms-detail-card mb-3">
        ...
        <div class="p-4 kms-detail-body">
            ...
            <div class="kms-detail-actions mb-3">
                ...
                @if($liked)
                    <form method="POST" action="{{ route('employee.sop.unlike', $sop) }}">
                        @csrf
                        @method('DELETE')
                        <button class="btn btn-outline-secondary"><i class="bi bi-heart-fill text-danger me-1"></i>{{ $sop->likes_count }}</button>
                    </form>
                @else
                    <form method="POST" action="{{ route('employee.sop.like', $sop) }}">
                        @csrf
                        <button class="btn btn-outline-secondary"><i class="bi bi-heart me-1"></i>{{ $sop->likes_count }}</button>
                    </form>
                @endif
            </div>
    ...
    <div class="kms-table-wrap p-4">
        ...
        <div class="d-grid gap-2 mb-3 kms-comments-list">
